...
ALTER PROCEDURE USP_RESUMEN_POR_CLIENTE
( @COD_CLIE CHAR(5), @ANIO INT )
AS
BEGIN -- Inicio del SP
	...
	JOIN Customers C ON O.CustomerID = C.CustomerID
	JOIN Products P ON OD.ProductID = P.ProductID
	WHERE C.CustomerID = @COD_CLIE AND YEAR(O.OrderDate) = @ANIO
	GROUP BY P.ProductID, P.ProductName
	ORDER BY 1;
...
EXEC USP_RESUMEN_POR_CLIENTE 'ANATR', 1997
GO
